...
function nextWeekdayOnOrAfter(todayIso, weekday) {
...
function extractWeekdayDate(text, today) {
...
export function extractInboundPromise(body, today) {
  ...
  if (explicit) {
    return { classification: "promise", promiseOn: explicit, summary, confidence: "high" };
  }

  const weekday = extractWeekdayDate(body, today);
  if (weekday && !/\bnext week\b/i.test(body)) {
    return { classification: "promise", promiseOn: weekday, summary, confidence: "high" };
  }
  if (weekday && /\bnext week\b/i.test(body)) {
    return { classification: "promise", promiseOn: weekday, summary, confidence: "low" };
  }
